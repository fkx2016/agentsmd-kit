/**
 * agentsmd-kit subscription Worker
 *
 * Endpoints:
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/health' && request.method === 'GET') {
      return json({ ok: true, service: 'agentsmd-kit-subscribe' }, 200, env);
    }
...
async function sendDownloadEmail(email, env) {
  ...
  const resp = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [email],
      subject: 'Your AGENTS.md Starter Kit',
      html,
      text,
      tags: [{ name: 'campaign', value: 'agentsmd-kit' }]
    })
  });
